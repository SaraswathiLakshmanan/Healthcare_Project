...
SELECT
    pc.city,
    COALESCE(rp.numRegisteredPeople, 0) AS numRegisteredPeople,
    COALESCE(ic.numInsuranceCompany, 0) AS numInsuranceCompany,
    COALESCE(pc.numPharmacy, 0) AS numPharmacy
FROM PharmacyCount pc 
FULL JOIN InsuranceCompanyCount ic ON pc.city = ic.city
FULL JOIN RegisteredPeopleCount rp ON pc.city = rp.city
ORDER BY numRegisteredPeople DESC;
